...
void ItemGOMarkovCommonCause2::paint(QPainter *painter, const QStyleOptionGraphicsItem*, QWidget*)
{
    ...
    painter->drawText(QRectF(5, 45, 200, 100), Qt::AlignTop | Qt::AlignLeft, QObject::tr("Belta: %1").arg(this->model()->belta()));
    painter->drawText(QRectF(5, 65, 200, 100), Qt::AlignTop | Qt::AlignLeft, QObject::tr("Gamma C: %1").arg(this->model()->gammaC()));
    painter->drawText(QRectF(5, 85, 200, 100), Qt::AlignTop | Qt::AlignLeft, QObject::tr("Operators: "));
    int y = 105;
...
{
    this->_end.setY(105 + 20 * this->_operatorItems->size() + 5);
}
